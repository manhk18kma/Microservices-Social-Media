...
//    @PreAuthorize("#idProfile == authentication.name and hasRole('USER')")
    @Operation(summary = "Check if there are unchecked notifications", description = "Check if there are any notifications that have not been checked for a specific profile")
    @GetMapping("/{idProfile}/exists-not-checked")
    public ResponseData<ExistNotCheckedResponse> existsNotificationNotChecked(
            @PathVariable @Parameter(description = "Profile ID for which notifications are retrieved", required = true) String idProfile) throws ExecutionException, InterruptedException {
        System.out.println("heree");
        ExistNotCheckedResponse response = notificationProfileService.existsNotificationNotChecked(idProfile);
