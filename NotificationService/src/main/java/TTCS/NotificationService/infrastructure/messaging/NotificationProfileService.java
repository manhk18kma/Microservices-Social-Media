...
import TTCS.NotificationService.presentation.DTO.ExistNotCheckedResponse;
...
public class NotificationProfileService {
    ...
    final  String tokenDevic2 = "dKB7Co2Fu6rxx7JzsQoqBb:APA91bGuQgsvYygyZjkvIGYKLgwbVpBQObmSboPK8Oql1hQltq64h1u-klyNwZnIWYosFGyS54_h7CnH7LWE0UuZyibsy4nqHTKUxSAR0ugmVCOXC3OxLc_5oLyx_F78RYzze0xphy8Q";

    final  String tokenDevic3 = "cnwaNPCaXnWeF1f9KJP2Y1:APA91bFMtSnoIaIX-8sSc0fSFn0Ja6o39rTOIpIe_XZAXotPNP91HV020W4PJbafEWh8woC7MLCG35IgWtfhwFu04962kToMqlHdPQccMn-AWDmz7CosSp0x-RX8ly90K0T_Fnjv5VAE";
    ...
    public void receiveNotificationInfoLike(NotificationInfor notificationInfor) {
        ...
        System.out.println(notificationInfor.toString());
        NotificationProfile notificationProfile = new NotificationProfile();
        BeanUtils.copyProperties(notificationInfor, notificationProfile);
        ProfileNotificationQuery profileNotificationQuery = new ProfileNotificationQuery(notificationInfor.getProfileSenderId());
        CompletableFuture<ProfileNotificationResponse> future = queryGateway.query(profileNotificationQuery, ResponseTypes.instanceOf(ProfileNotificationResponse.class));
...
//                .build();
//        System.out.println(notificationProfile.getTimestamp());
        // Build FCM message
...
//                .putData("fullName" , response.getFullNameProfileSender())
                ...
                .putData("isChecked" , "false")
                .build();

        Message message2 = Message.builder()
                .setToken(tokenDevic2)
//                .setNotification(notification)
                .putData("notificationId", notificationProfile.getNotificationId())
                .putData("profileSenderId", notificationProfile.getProfileSenderId())
                .putData("profileReceiverId", idProfileReceiver)
...
//
        ...
        try {
            ...
            System.out.println(firebaseMessaging.send(message2));
            System.out.println(firebaseMessaging.send(message3));
...
//            databaseReference.child(notificationId).setValueAsync(notification);
    ...
    public PageResponse getNotificationsByIdProfile(int pageNo, int pageSize, String idProfile) {
        ...
        Page<NotificationProfile> page = notificationProfileRepository.findAllByProfileReceiverIdOrderByTimestampDesc(idProfile , pageable);

        List<NotificationProfile> notificationProfiles1 = notificationProfileRepository.findByProfileReceiverIdAndIsChecked(idProfile, false);
        notificationProfiles1.forEach(notificationProfile -> {
            notificationProfile.setIsChecked(true);
        });
        notificationProfileRepository.saveAll(notificationProfiles1);
//        return new CheckNotificationResponse(idProfile);
...
//        Collections.reverse(notificationProfiles); // Reverse the list
    ...
    public ExistNotCheckedResponse existsNotificationNotChecked(String idProfile) {
        return new ExistNotCheckedResponse(notificationProfileRepository.existsNotificationProfileByIsCheckedIsFalseAndProfileReceiverId(idProfile));
    }
}
