...
import TTCS.IdentityService.application.Command.CommandEvent.Event.AccountResetPasswordEvent;
...
public class AccountEventService {
    ...
    @EventHandler
    public void on(AccountResetPasswordEvent event){
        Optional<Account> account = accountRepository.findById(event.getIdAccount());
        account.get().setPassword(event.getNewPassword());
        account.get().setUpdateAt(new Date());
        accountRepository.save(account.get());
    }
