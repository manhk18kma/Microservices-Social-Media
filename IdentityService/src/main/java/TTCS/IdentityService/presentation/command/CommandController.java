...
public class CommandController {
    ...
    public ResponseData<OTPResponse> generateOTP(
            ...
            @PathVariable("email") @NotNull @NotBlank(message = "ID must not be blank") String email
    ) throws ExecutionException, InterruptedException {
        CompletableFuture<OTPResponse> response = accountCommandService.generateOTP(email);
        return new ResponseData<>(HttpStatus.CREATED.value(), "Check your email", new Date(), response.get());
    }

    @Operation(summary = "Forgot password", description = "Send a request via this API to get the account password using the provided ID")
    @PutMapping("/{email}/resetPassword")
    public ResponseData<AccountCommandResponse> forgotPassword(
            @Parameter(description = "Account ID to change password", required = true)
            @PathVariable("email") @NotNull @NotBlank(message = "ID must not be blank") String email,
            @RequestBody @Valid ForgotPasswordReset forgotPasswordReset
    ) throws ExecutionException, InterruptedException {
        CompletableFuture<AccountCommandResponse> response = accountCommandService.forgotPassword(forgotPasswordReset , email);
        return new ResponseData<>(HttpStatus.OK.value(), "Password reset successfully", new Date(), response.get());
    }
}
