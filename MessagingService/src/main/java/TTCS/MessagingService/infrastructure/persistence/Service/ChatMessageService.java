...
import TTCS.MessagingService.Domain.Model.*;
...
public class ChatMessageService {
        ...
        sortedMap.forEach((s, date) -> {
            if (currentIndex.get() >= startIndex && currentIndex.get() < endIndex) {
                ...
                if (chatRoomOptional.isPresent()) {
                    ...
                    String lastMessage = chatMessageRepository.findFirstByidChatRoomChatDualOrderByTimeStampDesc(chatDual.getIdChatDual()).getContent();
                    ...
                    contactsResponses.add(
                            ContactsResponse.builder()
                                    ...
                                    .idProfile(profileMessageResponse.getIdProfile())
    ...
    public void setOfflineStatus(String idChatProfile) {
        ChatProfile chatProfile = profileChatRepository.findById(idChatProfile).orElseThrow(()->new AppException(AppErrorCode.CHAT_PROFILE_NOT_EXISTED));
