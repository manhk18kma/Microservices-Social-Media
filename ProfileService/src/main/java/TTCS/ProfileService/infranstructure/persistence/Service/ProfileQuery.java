...
public class ProfileQuery {
    ...
    public ProfileDetailResponse getById(String id, String idProfileToken) {
        Profile profileToken = profileRepository.findById(idProfileToken)
        ...
        Profile profile = profileRepository.findById(id)
                .orElseThrow(() -> new AppException(AppErrorCode.PROFILE_NOT_EXISTED));
        GetIdChatProfileQuery query = new GetIdChatProfileQuery(profile.getIdProfile());
        CompletableFuture<GetIdChatProfileResponse> future = queryGateway.query(query, ResponseTypes.instanceOf(GetIdChatProfileResponse.class));
        GetIdChatProfileResponse response = future.join();
        ...
        profileDetailResponse.setIdChatProfile(response.getIdChatProfile());

        profileDetailResponse.setCountFollowers(profile.getFollower().size());
        profileDetailResponse.setCountFollowings(profile.getFollowing().size());
        profileDetailResponse.setCountFriends(profile.getFriendShip().size());

        GetIdChatQuery getIdChatQuery = new GetIdChatQuery(
                idProfileToken,profile.getIdProfile()
        );
        CompletableFuture<GetIdChatResponse> future1= queryGateway.query(getIdChatQuery, ResponseTypes.instanceOf(GetIdChatResponse.class));
        GetIdChatResponse response1 = future1.join();
        profileDetailResponse.setIdChat(response1.getIdChat());
        ...
        } else if (friends.stream().anyMatch(friend ->
                (friend.getIdProfile1().equals(profile.getIdProfile()) && friend.getIdProfile2().equals(profileToken.getIdProfile())) ||
                        (friend.getIdProfile1().equals(profileToken.getIdProfile()) && friend.getIdProfile2().equals(profile.getIdProfile())))) {
            profileDetailResponse.setTypeRelationship(TypeRelationship.FRIEND);
        } else if (profile.getIdProfile().equals(profileToken.getIdProfile())) {
        ...
        List<SearchProfileResponse> searchResponses = page.getContent().stream().map(profile1 -> {
            ...
            if (followers.stream().anyMatch(follow -> follow.getIdProfileFollower().equals(profile1.getIdProfile()))) {
                ...
                response.setTypeRelationship(TypeRelationship.FOLLOWING);
                    ...
                    (friend.getIdProfile1().equals(profile.getIdProfile()) && friend.getIdProfile2().equals(profile1.getIdProfile())) ||
                            (friend.getIdProfile1().equals(profile1.getIdProfile()) && friend.getIdProfile2().equals(profile.getIdProfile())))) {
            ...
            response.setIdProfile(profile1.getIdProfile());
